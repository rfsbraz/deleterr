        ...
        for url in urls:
            ...
            if username and listname:
                ...
                if listname == 'favorited':
                    logger.warning("Traktpy does not support favorited movies. Skipping...")
                elif listname == 'watched':
                    logger.warning("Traktpy does not support watched movies. Skipping...")
                elif listname == 'collected':
                    logger.warning("Traktpy does not support collected movies. Skipping...")
                for m in movies:
                    items[int(m.get_key('tmdb'))] = {'trakt': m, "list": url}
            elif listname:
                movies = []
                if listname == 'popular':
                    movies = trakt.Trakt['movies'].popular(exceptions=True, per_page=1000)
...
def extract_info_from_url(url):
    ...
    user_list_pattern = r'https://trakt.tv/users/(?P<username>[^/]+)/lists/(?P<listname>[^/]+)'
    # Pattern to capture trending, popular movies
    movie_pattern = r'https://trakt.tv/movies/(?P<listname>trending|popular|anticipated|boxoffice)'
