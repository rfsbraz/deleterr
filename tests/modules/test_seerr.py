...
class TestResolveTautulliUsername:
    ...
    def test_manual_mapping_by_username(self):
        """Manual mapping matches by Seerr username."""
    ...
    def test_fallback_to_seerr_username(self):
        """Falls back to Seerr username when no plexUsername."""
        from app.media_cleaner import _resolve_tautulli_username
...
@pytest.mark.unit
class TestCheckExcludedSeerrRequesterWatch:
    """Test the check_excluded_seerr_requester_watch function for Seerr."""

    def test_no_config_returns_true(self):
        """No config means not excluded (actionable)."""
        from app.media_cleaner import check_excluded_seerr_requester_watch

        result = check_excluded_seerr_requester_watch(
    ...
    def test_disabled_returns_true(self):
        ...
        exclude = {"seerr": {"protect_unwatched_requesters": {"enabled": False}}}
        result = check_excluded_seerr_requester_watch(
            {"title": "Test", "tmdbId": 550}, MagicMock(), exclude, MagicMock(), MagicMock(), "1"
        )
        assert result is True

    def test_no_seerr_instance_returns_true(self):
        """No Seerr instance returns True (actionable, allows deletion)."""
        from app.media_cleaner import check_excluded_seerr_requester_watch

        exclude = {"seerr": {"protect_unwatched_requesters": {"enabled": True}}}
        result = check_excluded_seerr_requester_watch(
    ...
    def test_no_request_returns_true(self):
        """No request in Seerr returns True (actionable)."""
        from app.media_cleaner import check_excluded_seerr_requester_watch
        ...
        result = check_excluded_seerr_requester_watch(
            {"title": "Test", "tmdbId": 550}, MagicMock(), exclude, seerr, MagicMock(), "1"
        )
        assert result is True
    ...
    def test_unwatched_requester_protected(self):
        ...
        result = check_excluded_seerr_requester_watch(
            {"title": "Test Movie", "tmdbId": 550}, plex_item, exclude, seerr, tautulli, "1"
        )
        assert result is False  # Protected
    ...
    def test_plex_username_auto_match(self):
        ...
        check_excluded_seerr_requester_watch(
            {"title": "Test", "tmdbId": 550}, plex_item, exclude, seerr, tautulli, "1"
        )

        # Should use plexUsername, not seerr username
    ...
    def test_unmappable_user_protected_by_default(self):
        ...
        result = check_excluded_seerr_requester_watch(
            {"title": "Test", "tmdbId": 550}, MagicMock(), exclude, seerr, tautulli, "1"
        )
        assert result is False  # Protected
    ...
    def test_tv_show_uses_grandparent_key(self):
        ...
        check_excluded_seerr_requester_watch(
            media_data, plex_item, exclude, seerr, tautulli, "1"
        )
